...
class PillarsScatter(torch.autograd.Function):
    ...
    def forward(ctx, feats, coords, N):
        with no_jit_trace():
            return PillarsScatter.pts_middle_encoder(feats, coords, N)
    
    @staticmethod
    def symbolic(g, feats, coords, N):
        N_tensor = g.op("Constant", value_t=torch.tensor(N, dtype=torch.int32), device=feats.device)
        return g.op("com.microsoft::PillarScatterPlugin", feats, coords, N_tensor)
...
class TRTModel_pre(nn.Module):
    ...
    def forward(self,
        ...
        for sweep_index in range(num_sweeps):
            ...
            geom_xyz_ls.append(geom_xyz)
            fused_context_ls.append(fused_context.contiguous())
            

        return torch.stack(geom_xyz_ls, 1), torch.stack(fused_context_ls, 1)# (1,4,5,70,1,48,3), (1,4,5,70,1,48,160), 4 is sequence num, 5 is camera mum
...
def run_cli(model_class=BEVDepthLightningModel,
    ...
    trtModel_pre = TRTModel_pre(model).cuda()
    trtModel_post = TRTModel_post(model).cuda()
    with torch.no_grad():
        ...
        dynamic_axes = {
            ...
            'N_points': {0: 'n_sweeps'},
            'coords': {0: 'n_sweeps'},
            'geom_xyz': {1: 'n_sweeeps'},
            'fused_context': {1: 'n_sweeps'}
        }
        torch.onnx.export(
            trtModel_pre,
            pre_inputs,
