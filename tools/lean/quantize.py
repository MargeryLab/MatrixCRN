...
class QuantAdd(torch.nn.Module):
    def __init__(self, quantization = True):
        ...
        if quantization:
            self._input_quantizer = quant_nn.TensorQuantizer(QuantDescriptor(num_bits=8, calib_method="histogram")) #使用 8 位量化位宽和直方图法作为校准方法
            self._input_quantizer._calibrator._torch_hist = True #使用 PyTorch 的直方图进行量化校准
            self._fake_quant = True #使用伪量化
        self.quantization = quantization
